...
Return_code  tree_push_left  (Tree* tree, Node* node, Element_value new_element_value, Atom_type atom_type, bool isnew, ...) {
    ...
    switch (atom_type) {
        ...
        case LAT_OPERATION: node->left_son->element   = { { .val_op_code = new_element_value }, false }; break;
        case LAT_CONST:     node->left_son->element   = { { .val_double         = new_element_value }, false }; break;
        case LAT_VARIABLE:  node->left_son->element   = { { .val_str            = new_element_value }, false }; break;
...
/*
    ...
    do {
        ...
        switch (tree_iterator.current->atom_type) {
            ...
            case LAT_OPERATION: fprintf (dump_file,  "%d (aka \"%s\") (type Op_code)",
                                                                         tree_iterator.current->element.value.val_op_code,
                                                 _op_code_to_str (tree_iterator.current->element.value.val_op_code)
                                        ); break;

            case LAT_CONST:     fprintf (dump_file, "%lf (type double)", tree_iterator.current->element.value.val_double);         break;
            case LAT_VARIABLE:  fprintf (dump_file,  "%s (type string)", tree_iterator.current->element.value.val_str);            break;
    ...
    for (size_t depth = 0; depth <= given_tree_depth; depth++) {
        ...
        do {
            ...
            if (_isleaf (tree_iterator.current)) { 
                ...
                print_node_value (file, tree_iterator.current);
...
Return_code  print_node_value  (FILE* file, Node* node) {
    ...
    switch (node->atom_type) {
        ...
        case LAT_DEFS:   fprintf (file, "DEFS");  break;
        case LAT_NVAR:   fprintf (file, "NVAR %s",  node->element.value.val_str); break;
        case LAT_NFUNC:  fprintf (file, "NFUNC %s", node->element.value.val_str); break;
        case LAT_BLOCK:  fprintf (file, "BLOCK"); break;
        case LAT_ARG:    fprintf (file, "ARG");   break;
        case LAT_CALL:   fprintf (file, "CALL %s", node->element.value.val_str); break;
        case LAT_PAR:    fprintf (file, "PAR %s",  node->element.value.val_str); break;
        case LAT_OP:     fprintf (file, "%s", _op_code_to_str (node->element.value.val_op_code)); break;
        case LAT_SEQ:    fprintf (file, "SEQ");    break;
        case LAT_ASS:    fprintf (file, "=");      break;
        case LAT_WHILE:  fprintf (file, "WHILE");  break;
        case LAT_IF:     fprintf (file, "IF");     break;
        case LAT_BRANCH: fprintf (file, "BRANCH"); break;
        case LAT_RET:    fprintf (file, "RET");    break;
        case LAT_CONST:  fprintf (file, "%lf", node->element.value.val_double); break;
        case LAT_VAR:    fprintf (file, "%s",  node->element.value.val_str);    break;
